...
export default function ResetPasswordPage() {
  ...
  const pkceToken = Array.from(searchParams.keys()).find((key) =>
    key.startsWith("pkce_")
  );

  useEffect(() => {
    // Validate that we have the required tokens and correct type
    if (type === "recovery" && ((accessToken && refreshToken) || pkceToken)) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      const result: ActionResult = await resetPassword(
        formData,
        tokenToUse,
      ...
      if (result.success) {
        ...
        if (
          ...
          "shouldTriggerAuthChange" in result.data
        ) {
