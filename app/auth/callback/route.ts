...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    try {
      ...
      if (!user) {
        ...
        return NextResponse.redirect(`${origin}/login?error=no_user&message=Authentication failed`)
      }
      ...
      return NextResponse.redirect(`${origin}/login?error=unexpected_error&message=An unexpected error occurred`)
    }
  }
  ...
  return NextResponse.redirect(`${origin}/login?error=missing_code&message=Invalid authentication link`)
}
